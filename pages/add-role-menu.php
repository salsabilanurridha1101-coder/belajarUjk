                ...
                <div class="card-body">
                    ...
                    <form action="" method="post">
                        <div class="mb-3">
                            ...
                            <input placeholder="Enter Level Name" type="text" class="form-control w-50" value="<?php echo $rowEdit['name'] ?? '' ?>" readonly>
                            <input type="hidden" name="level_id" value="<?php echo $rowEdit['id'] ?? '' ?>">
                        </div>
                        <div class="mb-3">
                            <?php foreach ($rowMenus as $menu): ?>
                            <label for="">
                                <input type="checkbox" name="menu_id[]"  value="<?php echo $menu['id'] ?>"<?php echo in_array($menu['id'], $selectMenuIds) ? 'checked' : '' ?>><?php echo $menu['name'] ?>
                            </label><br>
                            <?php endforeach?>
                        </div>
